...
public class UnitRow extends DefaultTable {
    ...
    public UnitRow(final Unit unit) {
        ...
        truncatedLabel = new TruncatedLabel(LABEL_SIZE, unit.getName(), Assets.labelStyle14_lightGreen);
        ...
        Image icon = new Image(unit.getClassIcon());
        ...
        addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                if (unit.getActionPoints() > 0 || GdxgConstants.DEVELOPER_MODE) {
                    side.shiftUnitRow(thiz);
                } else {
                    Gdxg.clientUi.getInfoDialog().show("Could not transfer unit", "Unit has no Action points.");
                }
            }
        });
    }
