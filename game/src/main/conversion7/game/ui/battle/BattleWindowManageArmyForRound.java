...
public class BattleWindowManageArmyForRound extends CustomWindow {
    ...
    private void createRows() {
        ...
        Array<Unit> playerArmyUnits = null
//                ClientCore.core.battle.humanPlayerArmyLink.getUnits()
                ;
        for (int i = 0; i < playerArmyUnits.size; i++) {
            Unit unit = playerArmyUnits.get(i);
            // FIXME core.battle removed
            final BattleFigure battleFigure = null
//                    ClientCore.core.battle.getFigure(unit)
                    ;
            ...
            // icon
            Image image = new Image(unit.squad.getClassIcon());
            Cell iconCell = table.add(image).fill().left().width(ICON_SIZE);
            image.setScaling(Scaling.fit);
            mapFigureOnIconCell.put(battleFigure, iconCell);

            // params
//            table.add(new Label(valueOf(unit.squad.getMeleeDamage()), Assets.labelStyle18yellow)).center();
            table.add(new Label(valueOf(unit.squad.getMainParams().get(UnitParameterType.HEALTH)), Assets.labelStyle18yellow)).center();
//            table.add(new Label(valueOf(unit.squad.getDefence()), Assets.labelStyle18yellow)).center();
        }
    }
