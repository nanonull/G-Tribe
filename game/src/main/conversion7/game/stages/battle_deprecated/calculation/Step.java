...
import conversion7.game.stages.battle_deprecated.FigureStepParamSpeedComparator;
...
public class Step implements Progressive {
    ...
    public Step(Step previousStep) {
        ...
        for (FigureStepParams prevStepFigureParams : initialFiguresParams) {
            if (!prevStepFigureParams.zeroLifed) {
    ...
    public void addFigure(BattleFigure battleFigure, Cell cell) {
        ...
        if (cell.isSeized()) {
            throw new GdxRuntimeException("Impossible place " + battleFigure + " at " + cell + " because another figure exists there: " + cell.seizedBy.battleFigure + "!");
        } else {
    ...
    public void calculateActions() {
        ...
        for (int i = 0; i < figuresParamsList.size; i++) {
            figuresParamsList.get(i).speed = MathUtils.RANDOM.nextInt(100);
        }
